...
class PokerGame {
    ...
    async processBuyIns() {
        for (const [playerId, player] of this.players) {
            try {
                ...
                await logTransaction(null, playerId, this.buyIn, 'poker_buyin');
            } catch (e) {
    ...
    async promptPlayerAction() {
        ...
        try {
            ...
            if (confirmation.customId === 'poker_fold') {
                ...
                await this.handleRaiseRequest(confirmation, player, callAmount);
            }
        } catch (e) {
    ...
    async handleRaiseRequest(interaction, player, callAmount) {
        const minRaiseAdd = Math.max(this.bigBlind, this.currentBet);
        const totalBet = this.currentBet + minRaiseAdd;
        const cost = totalBet - player.currentBet;
    ...
    async handlePostGame() {
        for (const [playerId, player] of this.players) {
            if (player.chips > 0) {
                await withTransaction(async (client) => {
                    await client.query('UPDATE users SET balance = balance + $1 WHERE id = $2', [player.chips, playerId]);
                });
                await logTransaction(null, playerId, player.chips, 'poker_cashout');
            }
        }
