...
// PostgreSQL Connection Config - Optimized for Railway
const pool = new Pool({
...
async function initDatabase() {
    try {
        ...
        // Add columns to existing guilds table if missing
        await query(`
            DO $$ 
            BEGIN 
        ...
        // User items table (standardized name for consistency with user_stocks)
        await query(`
        ...
        // Market Listings: Explicitly allow NULLs for item_id/stock_id
        await query(`
        ...
        // Add last_dividend column to existing users
        await query(`
            DO $$ 
            BEGIN 
        ...
        // Migration: Add missing columns to market_listings (Ensure nullable)
        await query(`
            DO $$ 
            BEGIN 
        ...
        // Seed/Update Stocks (Strict Mode: Force Real Companies)
        const stocks = [
            ['NVDA', 'NVIDIA Corporation', 8500, 0.12],       // High volatility tech
            ['AAPL', 'Apple Inc.', 17500, 0.06],              // Stable blue chip
            ['MSFT', 'Microsoft Corporation', 38000, 0.07],   // Enterprise tech
            ['TSLA', 'Tesla Inc.', 20000, 0.15],              // High volatility EV
            ['AMZN', 'Amazon.com Inc.', 15000, 0.08],         // E-commerce
            ['GOOGL', 'Alphabet Inc.', 14000, 0.08],          // Tech/AI
            ['META', 'Meta Platforms Inc.', 4500, 0.10],      // Social media
            ['BTC', 'Bitcoin Index Fund', 45000, 0.20]        // Crypto proxy
        ];

        const realTickers = stocks.map(s => s[0]);

        // 1. Upsert Real Stocks
        ...
        // 2. Cleanup Goofy/Legacy Stocks
        // Find IDs of stocks that are NOT in our real list
...
async function getXpLeaderboard(limit = 10) {
    ...
    return res.rows.map(row => ({ ...row, xp: parseInt(row.xp), level: parseInt(row.level) })); // Parse BigInts
}
...
async function updateSavings(userId, amount) {
    await ensureUser(userId);
    // Postgres doesn't return changes by default for UPDATE unless checked. 
    // We can use RETURNING or check rowParam.
    // However, logic here is: update if savings + amount >= 0
...
async function repayLoan(loanId, userId) {
    return withTransaction(async (client) => {
        ...
        // Update credit score calls logic from outside transaction wrapper, might fail if nested query uses different client.
        // But since updateCreditScore uses `query()` (global pool), it's separate connection. 
        // This is safe but not atomic with this transaction. For now it's acceptable.

        return { success: true, amountPaid: amountDue };
    });
}

// Getters (synchronous logic dependent on DB calls)
...
async function updateGuildSettings(guildId, settings) {
    ...
    // Build query "UPDATE guilds SET k1=$1, k2=$2 WHERE id=$3"
...
// Export
module.exports = {
    pool, // Export pool instead of db
    query,
    initDatabase,
    withTransaction,
    updateBalance,
    ...
    // Sync helpers (need score passed in)
    getBankLoanLimitSync,
    getBankInterestRateSync,
    getLoanApprovalOddsSync,

    // Wrappers for old API compatibility where possible, but calls MUST be awaited now.
    ...
    // Guild config helpers for per-guild Aternos settings
